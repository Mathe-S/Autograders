...
const TEST_FILE = "algorithmTest.ts";
...
export async function runTests(
  ...
  try {
    ...
    try {
      let cmd: string;
      if (useInstructorTests) {
        cmd = `cd ${tmpTestDir} && npx mocha -r ts-node/register ${TEST_FILE} --reporter json`;
      } else {
        // Use c8 to collect coverage data for student tests
        // Run c8 and mocha separately to avoid mixing outputs
        cmd = `cd ${tmpTestDir} && npx c8 --include=${MAIN_FILE} --reporter=json --reporter=text --report-dir=coverage mocha -r ts-node/register ${TEST_FILE} > /dev/null 2> c8_output.txt && npx mocha -r ts-node/register ${TEST_FILE} ts --reporter json > mocha_output.json 2>&1 || echo "Tests completed with errors" >> c8_output.txt`;
      }
  ...
  try {
    ...
    if (await fsUtils.fileExists(coverageFilePath)) {
      ...
      for (const filePath in coverageData) {
        if (filePath.includes(MAIN_FILE)) {
...
async function setupInstructorTestsEnvironment(
  ...
  // Copy instructor's flashcards.ts and student's algorithm.ts
  await fsUtils.copyFile(
    path.join(rootDir, "instructor/src", "flashcards.ts"),
    path.join(tmpTestDir, "flashcards.ts")
  );
  await fsUtils.copyFile(
    path.join(studentDir, "src", "algorithm.ts"),
    path.join(tmpTestDir, "algorithm.ts")
  );

  // Fix imports in the instructor test file
  const testContent = (
    await fsUtils.readFile(path.join(rootDir, "instructor/test", TEST_FILE))
  )
    .replace(/from "\.\.\/src\/algorithm"/g, 'from "./algorithm"')
    .replace(/from "\.\.\/src\/flashcards"/g, 'from "./flashcards"');

  await fsUtils.writeFile(path.join(tmpTestDir, TEST_FILE), testContent);
}
...
async function setupStudentTestsEnvironment(
  ...
  await fsUtils.copyFile(
    path.join(rootDir, "instructor/src", MAIN_FILE),
    path.join(tmpTestDir, MAIN_FILE)
  );

  // Check if student test file exists
  const studentTestPath = path.join(studentDir, "test", TEST_FILE);
  ...
  const testContent = (await fsUtils.readFile(studentTestPath))
    .replace(/from ["']\.\.\/src\/algorithm["']/g, 'from "./algorithm"')
    .replace(/from ["']\.\/algorithm["']/g, 'from "./algorithm"')
    .replace(/from ["']\.\.\/src\/flashcards["']/g, 'from "./flashcards"')
    .replace(/from ["']\.\/flashcards["']/g, 'from "./flashcards"');

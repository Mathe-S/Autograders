...
export async function compareStudents(
  ...
  const filesToCompare = FILES_TO_COMPARE;
...
export async function checkDefaultImplementation(
  ...
  const instructorFile = path.join(rootDir, "instructor", "src", MAIN_FILE);
  const studentFile = path.join(submissionsDir, studentId, "src", MAIN_FILE);
  ...
  if (overallFileSimilarity > 0.95) {
    ...
    return {
      studentId,
      defaultFunctions: FUNCTIONS_TO_CHECK,
      wholeCopyPaste: true,
    };
  }
...
export async function analyzeSimilarity(
  ...
  // First, check default implementations for all students
  ...
  // Filter out students who used default implementations for most functions
  const nonDefaultStudents = students.filter((student) => {
    const defaultFuncs = defaultImplementations[student] || [];
    return defaultFuncs.length < 5; // Skip if most functions are default
  });

  console.log(
    `Performing similarity analysis on ${
      nonDefaultStudents.length
    } students (excluding ${
      students.length - nonDefaultStudents.length
    } with mostly default implementations)`
  );

  // Create all pairs of non-default students to compare
  const pairs: { student1: string; student2: string }[] = [];
  for (let i = 0; i < nonDefaultStudents.length; i++) {
    for (let j = i + 1; j < nonDefaultStudents.length; j++) {
      pairs.push({
        student1: nonDefaultStudents[i],
        student2: nonDefaultStudents[j],
      });
    }
  }

  console.log(`This will compare ${pairs.length} pairs of submissions`);
